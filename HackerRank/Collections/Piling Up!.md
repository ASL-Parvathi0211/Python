# Piling Up!
    ...
    Constraints
        ...
        1 <= side Length < 2 to the power of 31
    ...
    Explanation
        In the first test case, pick in this order: left - , right - , left - , right - , left - , right - 1.
...
def piling(d):
    last = float('inf')  # Start with an infinitely large "top" cube
    
    while d:
        if d[0] >= d[-1]:      # Pick the bigger cube first
            pick = d.popleft()
        else:
            pick = d.pop()
        
        if pick > last:        # If the picked cube is bigger than the top, fail
            return "No"
        last = pick             # Update the top cube
